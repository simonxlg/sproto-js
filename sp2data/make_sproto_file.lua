local input_file,head_file,output_file = ...
...
local function load(name,head)
	local t =""
	local files = {head,name}
	for i = 1 ,#files do
		local filename = string.format("%s", files[i])
...
assert(input_file)
local data = sparser.parse(load(input_file,head_file))
